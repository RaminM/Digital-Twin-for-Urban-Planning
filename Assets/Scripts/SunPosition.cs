...
public class SunPosition : MonoBehaviour
{
    ...
    private float dtUTC, LSTM, EoT, B, TC, LST, HRA, LT, Dec, Elevation, Azimuth, Zenith, Sunrise, Sunset, distance = 10000;
    ...
    void Update()
    {
        ...
        if (passedTime > 60)
        {
            ...
            uiManager.UpdateTimeDate(dateTime);
            passedTime = 0f;
            if(dateTime.Hour == 0 && sendDayEnd){
                spawnManager.GetComponent<SpawnManager>().SetDayEnd();
                sendDayEnd = false;
            }else{
                sendDayEnd =true;
            }
